...
class ApiService {
  ...
  private transformResponseMediaUrls(data: any): void {
    ...
    console.log('transformResponseMediaUrls called with:', typeof data, Array.isArray(data) ? 'array' : 'object');
    ...
    if (Array.isArray(data)) {
      ...
      Object.keys(data).forEach(key => {
        ...
        if (key.includes('url') || key.includes('Url')) {
          ...
          console.log('Transforming URL field:', key, 'from:', data[key]);
          data[key] = this.transformMediaUrl(data[key]);
          console.log('Transformed to:', data[key]);
        } else {
  ...
  async getRecordingAtTimestamp(camera: string, timestamp: number, window: number = 2): Promise<string | null> {
    try {
      ...
      console.log('Video URL before transformation:', response.data.data?.video_url);
      
      if (response.data.success && response.data.data) {
        const originalUrl = response.data.data.video_url;
        console.log('Original video URL:', originalUrl);
        
        // Always transform the URL, regardless of interceptor
        const transformedUrl = this.transformMediaUrl(originalUrl);
        console.log('Transformed video URL:', transformedUrl);
        
        // Also check if interceptor already transformed it
        if (originalUrl !== transformedUrl) {
          console.log('✅ URL was transformed by getRecordingAtTimestamp');
        } else {
          console.log('❌ URL was NOT transformed by getRecordingAtTimestamp');
        }
