...
app.get('/v1/api/employees/work-hours', (req, res) => {
  const timezone = req.query.timezone || 'PKT';
  ...
  const tzInfo = timezoneMap[timezone] || timezoneMap['PKT'];
  
  // Convert employee times to the requested timezone
  const convertedEmployees = mockEmployees.map(emp => {
    const arrivalDate = new Date(emp.arrival_time);
    const departureDate = new Date(emp.departure_time);
    
    return {
      ...emp,
      arrival_time: arrivalDate.toLocaleString('sv-SE', { timeZone: tzInfo.timezone }).replace(' ', 'T') + tzInfo.offset,
      departure_time: departureDate.toLocaleString('sv-SE', { timeZone: tzInfo.timezone }).replace(' ', 'T') + tzInfo.offset
    };
  });
  ...
  res.json({
    ...
    data: {
      ...
      total_employees: convertedEmployees.length,
      total_work_hours: convertedEmployees.reduce((sum, emp) => sum + emp.total_work_hours, 0),
      average_work_hours: convertedEmployees.reduce((sum, emp) => sum + emp.total_work_hours, 0) / convertedEmployees.length,
      period: {
        start: `2025-10-20T00:00:00${tzInfo.offset}`,
        end: `2025-10-20T23:59:59${tzInfo.offset}`,
        duration_hours: 24
      },
    ...
    data: {
      employees: mockEmployees.map(emp => {
        const breakSessions = [
          ...
          {
            break_start: "2025-10-20T12:00:00+05:00",
            break_end: "2025-10-20T12:30:00+05:00",
            duration_hours: 0.5,
            previous_session: {
              camera: emp.cameras[0],
              ended_at: "2025-10-20T12:00:00+05:00"
            },
            next_session: {
              camera: emp.cameras[0],
              started_at: "2025-10-20T12:30:00+05:00"
            }
          },
          {
            break_start: "2025-10-20T15:45:00+05:00",
            break_end: "2025-10-20T16:00:00+05:00",
            duration_hours: 0.25,
            previous_session: {
              camera: emp.cameras[0],
              ended_at: "2025-10-20T15:45:00+05:00"
            },
            next_session: {
              camera: emp.cameras[0],
              started_at: "2025-10-20T16:00:00+05:00"
            }
          }
        ];
        
        const totalBreakTime = breakSessions.reduce((sum, session) => sum + session.duration_hours, 0);
        ...
        return {
          ...
          average_break_duration: totalBreakTime / breakSessions.length,
          longest_break: Math.max(...breakSessions.map(s => s.duration_hours)),
          shortest_break: Math.min(...breakSessions.map(s => s.duration_hours)),
          break_frequency: breakSessions.length / emp.total_work_hours,
          break_efficiency: 80.5,
          break_sessions: breakSessions
        };
      }),
          total_employees: mockEmployees.length,
          total_break_time: 2.0,
          average_break_time: 1.0,
          timezone_info: tzInfo
    }
  });
});
