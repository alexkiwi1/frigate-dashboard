...
class ApiService {
  ...
  async getTimezoneInfoFromAPI(timezone: string): Promise<TimezoneInfo> {
  ...
  processTimelineEventsToWorkHours(events: any[], timezone: string): any[] {
    ...
    const personEvents = new Map<string, any[]>();
    ...
    personEvents.forEach((events: any[], personId: string) => {
      ...
      const cameras = [...new Set(events.map((e: any) => e.camera))];
      const zones = [...new Set(events.flatMap((e: any) => e.zones))];
      ...
      employees.push({
        ...
        cameras,
        zones: zones.filter((z: any) => z && z !== 'employee_area')
      });
    });
